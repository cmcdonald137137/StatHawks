...
``` 
```{r}
# Previous Model from Computational
selected_vars <- c("IMQ020", "RIAGENDR", "RIDAGEYR", "RIDRETH3", "DMDEDUC2", "INDFMPIR", "HEQ010")
dfm <- combined_data_full[selected_vars]

# Convert categorical variables to dummy/indicator variables
dfm <- model.matrix(~ . - 1, data = dfm)
colnames(dfm)[colnames(dfm) == "HEQ0101"] <- "HEQ010"
...
train_dfm <- as.data.frame(train_dfm)
test_dfm <- dfm[-train_index, ]
test_dfm <- as.data.frame(test_dfm)
head(test_dfm)
test_heq <- subset(test_dfm, select = -c(HEQ010))
log_model_heq <- glm(HEQ010 ~ ., family = binomial(link = "logit"), data = train_dfm)

# Predict on the test set
log_pred_heq_test <- predict(log_model_heq, newdata = test_dfm, type = "response")

# Convert predicted probabilities to binary predictions
threshold <- 0.02
test_dfm$pred_heq <- ifelse(log_pred_heq_test > threshold, 1, 0)

# Calculate accuracy for the 'heq' binary classification
accuracy_heq <- mean(test_dfm$HEQ010 == test_dfm$pred_heq, na.rm = TRUE)
print(paste("Accuracy for 'heq':", accuracy_heq))
...
conf_matrix_heq <- confusionMatrix(data = factor(test_dfm$pred_heq, levels = c(0, 1)),
                                   reference = factor(test_dfm$HEQ010, levels = c(0, 1)))

# Print the confusion matrix
print("Confusion Matrix:")
print(conf_matrix_heq)
```
